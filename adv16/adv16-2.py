...
def get_energy(start_tuple, dir):
    beams = deque([ (start_tuple, dir) ])
    visited = set()
...
for c in range(len(grid[0])):
    max_energy = max(max_energy, get_energy((-1,c), "D"))
    max_energy = max(max_energy, get_energy((len(grid),c), "D"))

for r in range(len(grid)):
     max_energy = max(max_energy, get_energy((r,-1), "R"))
     max_energy = max(max_energy, get_energy((r,len(grid[0])), "L"))
print(max_energy)
